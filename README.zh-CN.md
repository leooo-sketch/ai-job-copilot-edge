...
**在 Edge 侧边栏完成岗位筛选、简历 AI 匹配，以及一键填写可靠匹配的网申资料。**
...
| 版本 | 分类 | 版本内容 |
|---|---|---|
| `v0.8.0` | 最新版本 | 资料库 v4：扩展个人基础信息、求职问答和实习证明人字段，并修复精确标签误判；[升级说明](CHANGELOG.md) |
| `v0.7.0` | 一键填充版 | 一键扫描并填写已保存资料、可搜索自定义下拉框增强；[升级说明](CHANGELOG.md) |
| `v0.6.1` | 下拉兼容版 | 旧版下拉控件兼容、学历同义匹配、培养方式与统招字段独立；[升级说明](docs/AUTOFILL-0.6.1.md) |
| `v0.6.0` | 网申填充增强 | 分离年月、报错文字过滤、填写回读核验、资料对应关系修正、奖项证书卡片；[保留资料升级说明](docs/AUTOFILL-0.6.md) |
| `v0.5.0` | 增强版 | 模糊字段识别、DOM 经历卡片分组、职位联想自动确认、原生/ARIA/主流框架单选控件兼容 |
| `v0.1.0` | 首个公开版 | Edge 侧边栏、招聘网站适配、AI 匹配、人工审核队列和安全投递流程 |
| `v0.4.1` | 可靠填充版 | 可靠网申填充、资料库 v3、多简历本地解析、千问复核、动态经历卡片和兼容性修复 |
...
- 通用网申填充：在普通 HTTP(S) 网申页面点击“一键扫描并填写”，自动写入资料库中可靠匹配的字段；也可仅扫描预览、取消勾选后再填写。
- 完整资料库 v4：增加年龄、家乡、现住址、国家/地区、职业资格、期望薪资、紧急电话、求职方式、考研/导师及亲友任职问答等独立字段；教育、工作、实习、项目仍按独立卡片保存，并补充经历证明人信息。
- 升级保留原有答案：继续使用同一扩展的本地存储键，v3 资料会在读取时迁移；升级前建议先导出 JSON 备份，勿先卸载扩展。
...
- 经历卡片隔离：按网页真实 DOM 卡片划分第 1/2/3 段经历，预览中显示“项目经历 2「项目名」· 字段名”，避免同一段经历散落到多个框格。
- 异步选项确认：职位关键词等可编辑下拉会先输入、等待候选、匹配唯一选项并确认；失败时恢复原值，不把未确认文字当作成功。
- 选择控件兼容：支持没有 `name` 的原生单选、ARIA `radio/radiogroup`，以及 Ant Design、Element、Arco、Semi、iView 常见单选结构和中英文等价选项。
- 多证据模糊识别：综合 ARIA 标签/说明、邻近文字和安全字段属性；轻微错别字可以给出保守建议，模糊项仍须人工勾选。
...
- 安全默认值：可靠匹配且资料库有值的敏感字段参与一键填写；不覆盖网页已有内容，不猜测缺失或有歧义的资料，不自动选择附件，不点击提交按钮。
...
3. 打开企业网申页，点击 **一键扫描并填写**。扩展会先按资料条数展开教育、工作、实习和项目卡片，再扫描、匹配并填写；只在你点击时申请当前域名权限。若希望先核对预览，可点击 **仅扫描预览**。
4. 本地规则先映射，启用后千问再复核语义。高置信度且已有资料的字段（包括敏感字段）会默认选中；模糊项和跨栏目项仍需人工处理。
5. 一键完成后检查网页；模糊匹配或跨栏目字段可以在侧边栏手动选择，再点 **填写已识别字段**。最终提交仍由你操作。
